...
timeout 60 aws ec2 run-instances \
    --image-id $AMI_ID \
    ...
    --region $REGION \
    --output json > $LAUNCH_OUTPUT 2>&1

LAUNCH_EXIT_CODE=$?

if [ $LAUNCH_EXIT_CODE -eq 124 ]; then
    echo -e "${RED}❌ Error: EC2 launch command timed out${NC}"
    echo "This might be a networking or API issue. Please try again."
    rm -f user-data.sh $LAUNCH_OUTPUT
    exit 1
elif [ $LAUNCH_EXIT_CODE -ne 0 ]; then
    echo -e "${RED}❌ Error launching EC2 instance:${NC}"
    cat $LAUNCH_OUTPUT
    rm -f user-data.sh $LAUNCH_OUTPUT
    exit 1
fi
...
if [ -z "$INSTANCE_ID" ] || [ "$INSTANCE_ID" == "null" ]; then
    echo -e "${RED}❌ Error: Failed to get instance ID${NC}"
    exit 1
fi
